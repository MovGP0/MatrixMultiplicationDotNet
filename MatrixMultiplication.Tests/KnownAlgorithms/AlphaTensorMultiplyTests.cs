...
public class AlphaTensorMultiplyTests
{
    ...
    public void MultiplyAlpha_MatchesNaive_DefaultPlanner(int n, int m, int p)
    {
        ...
        var A2 = ToArray2D(A);
        var B2 = ToArray2D(B);
        var C2 = AlphaTensorFormulaBuilder.MultiplyAlpha(A2, B2); // default thresholds
    ...
    public void MultiplyAlpha_MatchesNaive_VariousPlannerSettings(int n, int m, int p)
    {
        ...
        {
            var C2 = AlphaTensorFormulaBuilder.MultiplyAlpha(A2, B2, cfg.baseThreshold, cfg.idealBlock);
            var actual = FromArray2D(C2);
            AssertMatrixAlmostEqual(expected, actual, tol: 0.0);
        }
    }
